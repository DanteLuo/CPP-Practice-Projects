...
class sorted_priority_queue: public eecs281priority_queue<TYPE, COMP> {
	public:
		...
		sorted_priority_queue(InputIterator start, InputIterator end,
				COMP comp = COMP());
...
template<typename InputIterator>
sorted_priority_queue<TYPE, COMP>::sorted_priority_queue(InputIterator start,
...
template<typename TYPE, typename COMP>
void sorted_priority_queue<TYPE, COMP>::update_priorities() 
{
...
void sorted_priority_queue<TYPE, COMP>::push(const TYPE& val) 
{
    ...
    data.insert(data.begin()+lower_bound_index,val);
}
